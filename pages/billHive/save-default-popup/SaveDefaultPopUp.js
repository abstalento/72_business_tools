...
const SaveDefault = (props) => {
  ...
  const saveDefaultValue = () => {
    // localStorage.setItem('billHiveDefault', JSON.stringify(props.defaultStore))
    var db = new PouchDB("BillHive");
    db.get("billHiveDefault", function (err, doc) {
      if (err) {
        var doc = {
          _id: "billHiveDefault",
  ...
  return (
    ...
    <Dialog fullWidth={fullWidth}
        ...
        <div
          className='mx-auto w-[100%] border-2'
        >
          ...
          <div className='pb-2 flex justify-evenly'>
            <h1 style={{ width: '65%', fontFamily: 'sfpro-medium' }} className=' p-3 text-center'>
              {/* {deleteContent} */}Do you want to make this invoice as the default template
            </h1>
          </div>

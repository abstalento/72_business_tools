...
const HistoryPage = (props) => {
  ...
  const deleteInvoice = (data, index) => {
    if (confirm("Are you sure you want to delete this invoice?") === true) {
      ...
      // localStorage.setItem("InvoiceHistory", JSON.stringify(localStoreData));
      var db = new PouchDB("invoiceGenerator");
  ...
  const  sampleDataSet=(invoiceHistory)=>{
    const invoiceData = [];
    setGetMyItem(invoiceHistory.data)
    if (invoiceHistory.data) {
      setLocalStoreData(invoiceHistory.data);
      setSearchData(invoiceHistory.data);
      invoiceHistory.data.map((gitem) => {
        ...
        itemList.map((value) => {
          const { amount, item, quality, rate, iGst, cGst, sGst, netTotal} = value;
          invoiceData.push({
            currencyValue,
            amountPaid,
            balanceDue,
  ...
  useEffect(() => {
    (async function Change(){
      await Service.invoiceGeneratorHistory()
    let invoice=await Service.invoiceGeneratorHistory()
      sampleDataSet(invoice)
    })()
    // const invoiceData = [];
    // const getItem = JSON.parse(localStorage.getItem("InvoiceHistory"));
    // setGetMyItem(getItem);
    // if (getItem) {
    //   setLocalStoreData(getItem);
    ...
    //     } = itemDetails;
    //     itemList.map((value) => {
    //       const { amount, item, quality, rate, iGst, cGst, sGst, netTotal } =
    //         value;
    //       invoiceData.push({
    //         currencyValue,
    //         amountPaid,
  ...
  return (
    <div className="h-[100vh] flex flex-col items-center bg-[#E6E9ED] justify-center">
      <div className="w-[88%] md:w-[45%] mx-auto bg-white p-5  h-[70vh] lg:min-h-[70vh] overflow-y-scroll scrollBar flex flex-col justify-between rounded-lg">
        <div>
          ...
          <div className="flex justify-between pb-5">
            <div className="flex pl-2 items-center rounded-md border-2 bg-[#E6E9ED] w-[100%] h-[40px]">
              ...
              <input
                id="Testsearch"
                className="bg-[#E6E9ED] w-[100%] pl-2 focus:outline-none"
                placeholder="search"
              ...
              {localStoreData.map((data, index) => {
                return (
                  <div
                    key={index}
                    id="invoiceDetails"
                    className="group flex flex-col mr-[20px] w-[36%] lg:w-[27%] xl:w-[20%] h-[130px] mb-4 rounded-lg border-2 border-[#009e74] bg-[#F1FAF9]"
                  >
                    <div
                      id={index}
        ...
        <div>
          <div className="flex justify-between float-right w-[73%] lg:w-[53%] xl:w-[40%]">
            <div
              className={`${
                !getMyItem?.length == null || !getMyItem?.length == 0
                  ? "text-[#009e74]"
